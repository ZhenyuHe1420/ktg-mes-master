...
public class ProTaskController extends BaseController
{
            ...
            for (ProWorkorder workorder: workorders
                ...
                if(CollUtil.isNotEmpty(proTasks)){
                    for (ProTask task:proTasks
                        ...
                        data.setProgress(taskproduced.divide(taskquantitiy,BigDecimal.ROUND_HALF_UP).setScale(4,BigDecimal.ROUND_HALF_UP).floatValue());
    ...
    public AjaxResult add(@RequestBody ProTask proTask)
    {
        ...
        proTask.setUnitName(order.getUnitName());
    ...
    public AjaxResult edit(@RequestBody ProTask proTask)
    {
        ...
        if(UserConstants.ORDER_STATUS_FINISHED.equals(proTask.getStatus())){
            proTask.setFinishDate(new Date());
        }

        if(UserConstants.ORDER_STATUS_CANCELED.equals(proTask.getStatus())){
            proTask.setCancelDate(new Date());
        }
