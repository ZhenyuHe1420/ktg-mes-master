...
public class ProRouteController extends BaseController
{
    ...
    public AjaxResult add(@RequestBody ProRoute proRoute)
    {
        ...
        proRoute.setCreateBy(getUsername());
    ...
    public AjaxResult edit(@RequestBody ProRoute proRoute)
    {
        ...
        //启用工艺路线时的校验
        if(UserConstants.YES.equals(proRoute.getEnableFlag())){
            //必须添加了工序
            List<ProRouteProcess> processesList = proRouteProcessService.selectByRouteIds(Arrays.asList(proRoute.getRouteId()));
            if(CollectionUtils.isEmpty(processesList)){
                return AjaxResult.error("请添加组成工序！");
            }
            //必须要有关键工序
            List<ProRouteProcess> keyProcessesList = processesList.stream().filter(p -> UserConstants.YES.equals(p.getKeyFlag())).collect(Collectors.toList());;
            if(keyProcessesList.size() == 0){
                return AjaxResult.error("工艺路线必须要有关键工序！");
            }
            //各个产品必须在工序中配置了BOM消耗
            ProRouteProduct product = new ProRouteProduct();
            product.setRouteId(proRoute.getRouteId());
            List<ProRouteProduct> productList = proRouteProductService.selectProRouteProductList(product);
            if(!CollectionUtils.isEmpty(productList)){
                StringBuilder sb = new StringBuilder();
                boolean flag = true;
                for (ProRouteProduct p : productList){
                    ProRouteProductBom bom = new ProRouteProductBom();
                    bom.setRouteId(proRoute.getRouteId());
                    bom.setProductId(p.getItemId());
                    List<ProRouteProductBom> bomList = proRouteProductBomService.selectProRouteProductBomList(bom);
                    if(CollectionUtils.isEmpty(bomList)){
                        flag = false;
                        sb.append("产品：").append(p.getItemName()).append("未配置工序的BOM消耗!");
                    }
                }
                if(!flag){
