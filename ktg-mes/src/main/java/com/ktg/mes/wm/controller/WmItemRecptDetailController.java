...
public class WmItemRecptDetailController extends BaseController
{
    ...
    @GetMapping("/list")
    public TableDataInfo list(WmItemRecptDetail wmItemRecptDetail)
    {
        startPage();
     ...
     * 导出物料入库单明细列表
     */
    @PreAuthorize("@ss.hasPermi('mes:wm:itemrecept:export')")
    @Log(title = "物料入库单明细", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, WmItemRecptDetail wmItemRecptDetail)
    {
        List<WmItemRecptDetail> list = wmItemRecptDetailService.selectWmItemRecptDetailList(wmItemRecptDetail);
        ExcelUtil<WmItemRecptDetail> util = new ExcelUtil<WmItemRecptDetail>(WmItemRecptDetail.class);
        util.exportExcel(response, list, "物料入库单明细数据");
    }
