...
public class WmOutsourceRecptDetailController extends BaseController
{
    @Autowired
    ...
    @Log(title = "外协入库单明细", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, WmOutsourceRecptDetail wmOutsourceRecptDetail)
    {
        List<WmOutsourceRecptDetail> list = wmOutsourceRecptDetailService.selectWmOutsourceRecptDetailList(wmOutsourceRecptDetail);
        ExcelUtil<WmOutsourceRecptDetail> util = new ExcelUtil<WmOutsourceRecptDetail>(WmOutsourceRecptDetail.class);
        util.exportExcel(response, list, "外协入库单明细数据");
    }
