...
public class StorageCoreServiceImpl implements IStorageCoreService {
    ...
    public void processMiscRecpt(List<MiscRecptTxBean> lines) {
        String transactionType = UserConstants.TRANSACTION_TYPE_MISC_RECPT;
        if(CollUtil.isEmpty(lines)){
            throw new BussinessException("没有需要处理的入库单行");
        }

        for (int i =0;i<lines.size();i++){
            MiscRecptTxBean line = lines.get(i);
            WmTransaction transaction = new WmTransaction();
            transaction.setTransactionType(transactionType);
            BeanUtils.copyBeanProp(transaction,line);
            transaction.setTransactionFlag(1); //库存增加
    ...
    public void processMiscIssue(List<MiscIssueTxBean> lines) {
        ...
        String transactionType = UserConstants.TRANSACTION_TYPE_MISC_ISSUE;
        for(int i=0;i<lines.size();i++){
            MiscIssueTxBean bean = lines.get(i);
            WmTransaction transaction = new WmTransaction();
            transaction.setTransactionType(transactionType);
    ...
    public void processProductSales(List<ProductSalesTxBean> lines) {
        ...
        for(int i=0;i<lines.size();i++){
            ProductSalesTxBean bean = lines.get(i);
            WmTransaction transaction = new WmTransaction();
            transaction.setTransactionType(transactionType);
            BeanUtils.copyBeanProp(transaction,bean);
            transaction.setTransactionFlag(-1); //库存减少
    ...
    public void processRtSales(List<RtSalesTxBean> lines) {
        ...
        for(int i=0;i<lines.size();i++){
            RtSalesTxBean bean = lines.get(i);
            WmTransaction transaction = new WmTransaction();
            transaction.setTransactionType(transactionType);
            BeanUtils.copyBeanProp(transaction,bean);
            transaction.setTransactionFlag(1); //库存增加
