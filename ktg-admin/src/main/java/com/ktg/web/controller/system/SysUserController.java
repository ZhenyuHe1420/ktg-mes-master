...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult edit(@Validated @RequestBody SysUser user)
    {
        ...
        if (!sysUser.getNickName().equals(user.getNickName())) {
            if (StringUtils.isNotEmpty(user.getNickName()) && UserConstants.NOT_UNIQUE.equals(userService.checkNickNameUnique(user))) {
                return AjaxResult.error("修改用户'" + user.getUserName() + "'失败，昵称已存在");
            }
        }
